...
function initLetters2() {
  letters2 = []; // Asegúrate de limpiar letters2 antes de inicializar
  for (var i = 0; i < opts2.strings.length; ++i) {
    for (var j = 0; j < opts2.strings[i].length; ++j) {
      letters2.push(
        new Letter(
          opts2.strings[i][j],
          j * opts2.charSpacing +
            opts2.charSpacing / 2 -
            (opts2.strings[i].length * opts2.charSize) / 2,
          i * opts2.lineHeight +
            opts2.lineHeight / 2 -
            (opts2.strings.length * opts2.lineHeight) / 2
        )
      );
    }
  }
...
function anim() {
  ...
  drawStars(); // Dibuja las estrellas
  ...
  if (done && !showSecondText) {
    showSecondText = true;
    initLetters2(); // Inicializa el segundo texto
  }

  if (showSecondText) {
    var done2 = true;
    for (var l = 0; l < letters2.length; ++l) {
      letters2[l].step();
      if (letters2[l].phase !== "done") done2 = false;
    }
  }

  ctx.translate(-hw, -hh);

  if (done && showSecondText && done2) {
    showSecondText = false; // Reinicia el estado para la próxima animación
    for (var l = 0; l < letters.length; ++l) letters[l].reset();
    letters2 = []; // Limpia letters2 después de la animación
    createShootingStar(); // Crea una estrella fugaz después de la animación
  }

  drawShootingStar(); // Dibuja la estrella fugaz
}
